...
/// Languages the interface can be shown in. EazyDisplay follows the system unless one is chosen in the app.
enum AppLanguage: String, CaseIterable, Identifiable, Sendable {
    ...
    /// The saved choice. A per-app language picked in System Settings counts as one until EazyDisplay saves its own.
    ...
    /// The language the system settings pick among the ones EazyDisplay has.
    ...
    /// Uses this language for every EazyDisplay string from now on.
    func apply() {
        let table = StringTable(self)
        activeTable.withLock { $0 = table }
    }

    /// Remembers the choice and hands it to macOS, so text the system draws for EazyDisplay follows it from the next
    /// launch.
    func save() {
...
/// The interface language; views that read it redraw when it changes.
@MainActor
@Observable
...
private let resources = Bundle.main.resourceURL
    .flatMap { Bundle(url: $0.appendingPathComponent("EazyDisplay_EazyDisplay.bundle")) } ?? .module
