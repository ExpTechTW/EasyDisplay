...
/// Checks GitHub for a newer EazyDisplay and replaces this copy with it.
///
...
final class Updater {
    ...
    private(set) var preferences = UpdatePreferences.load()

    @ObservationIgnored let build = BuildInfo.current
    @ObservationIgnored private let appURL = Bundle.main.bundleURL.resolvingSymlinksInPath()
    @ObservationIgnored private let team = UpdateInstaller.signingTeam()
    @ObservationIgnored private var timer: Timer?
    @ObservationIgnored private var panel: WarningPanel?
    ...
    private var userAgent: String { "EazyDisplay/\(build.label)" }
    ...
    func start() {
        ...
        self.timer = timer
        // Every launch checks, once the displays have settled.
        Task { [weak self] in
    ...
    func check(userInitiated: Bool = true) {
        ...
        Task {
            do {
                ...
                #if DEBUG
                if available != nil, ProcessInfo.processInfo.environment["EAZYDISPLAY_UPDATE_AUTOINSTALL"] != nil {
                    return install()
                }
                #endif
...
enum UpdateInstaller {
    ...
    private static let api = ProcessInfo.processInfo.environment["EAZYDISPLAY_UPDATE_API"] ?? "https://api.github.com"
    #else
