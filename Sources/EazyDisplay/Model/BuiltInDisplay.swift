...
/// boosted auto-brightness on, EazyDisplay does its own from the ambient light sensor instead: an `AmbientFilter`
...
/// room) while EazyDisplay kept writing it back, a visible flicker. Brightness keys reach EazyDisplay directly.
///
...
final class BuiltInDisplay: Identifiable {
    ...
    /// Direct backlight writes bypass corebrightnessd's thermal management, so EazyDisplay applies its own, well
    ...
    /// Auto-brightness as it affects the display now: EazyDisplay's own while boosted, the system's otherwise.
    ...
    /// adjustment, or macOS moving its slider (Control Center, or a brightness key EazyDisplay couldn't take). If the
