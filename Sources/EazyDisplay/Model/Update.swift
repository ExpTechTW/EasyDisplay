...
struct BuildInfo: Equatable, Sendable {
    ...
    init(info: [String: Any]) {
        let label = info["EazyDisplayLabel"] as? String
        self.label = label ?? "dev"
        code = label == nil ? 0 : (info["CFBundleVersion"] as? String).flatMap { Int($0) } ?? 0
        date = info["EazyDisplayDate"] as? String
        isPrerelease = info["EazyDisplayPrerelease"] as? Bool ?? true
        repository = (info["EazyDisplayRepository"] as? String).flatMap {
...
struct Release: Decodable, Equatable, Sendable {
    ...
    /// The build it carries, from the `<!-- eazydisplay-build: 126000042 -->` line at the end of its notes.
    var code: Int? {
        guard let body, let match = body.firstMatch(of: #/<!--\s*eazydisplay-build:\s*(\d+)\s*-->/#) else { return nil }
        return Int(match.1)
    }

    /// The zipped app, `EazyDisplay-<label>.zip`.
    var archive: Asset? {
        assets.first { $0.name.hasPrefix("EazyDisplay-") && $0.name.hasSuffix(".zip") }
    }
}
...
struct UpdatePreferences: Codable, Equatable, Sendable {
    ...
    private static let key = "EazyDisplay.update.v1"
