...
struct BuildInfo: Equatable, Sendable {
    ...
    init(info: [String: Any]) {
        let label = info["EasyDisplayLabel"] as? String
        self.label = label ?? "dev"
        code = label == nil ? 0 : (info["CFBundleVersion"] as? String).flatMap { Int($0) } ?? 0
        date = info["EasyDisplayDate"] as? String
        isPrerelease = info["EasyDisplayPrerelease"] as? Bool ?? true
        repository = (info["EasyDisplayRepository"] as? String).flatMap {
...
struct Release: Decodable, Equatable, Sendable {
    ...
    /// The build it carries, from the `<!-- easydisplay-build: 126000042 -->` line at the end of its notes.
    var code: Int? {
        guard let body, let match = body.firstMatch(of: #/<!--\s*easydisplay-build:\s*(\d+)\s*-->/#) else { return nil }
        return Int(match.1)
    }

    /// The zipped app, `EasyDisplay-<label>.zip`.
    var archive: Asset? {
        assets.first { $0.name.hasPrefix("EasyDisplay-") && $0.name.hasSuffix(".zip") }
    }
}
...
struct UpdatePreferences: Codable, Equatable, Sendable {
    ...
    private static let key = "EasyDisplay.update.v1"
