...
/// Languages the interface can be shown in. EasyDisplay follows the system unless one is chosen in the app.
enum AppLanguage: String, CaseIterable, Identifiable, Sendable {
    ...
    /// The saved choice. A per-app language picked in System Settings counts as one until EasyDisplay saves its own.
    ...
    /// The language the system settings pick among the ones EasyDisplay has.
    ...
    /// Uses this language for every EasyDisplay string from now on.
    func apply() {
        let table = StringTable(self)
        activeTable.withLock { $0 = table }
    }

    /// Remembers the choice and hands it to macOS, so text the system draws for EasyDisplay follows it from the next
    /// launch.
    func save() {
...
private let resources = Bundle.main.resourceURL
    .flatMap { Bundle(url: $0.appendingPathComponent("EasyDisplay_EasyDisplay.bundle")) } ?? .module
