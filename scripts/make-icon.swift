...
func render(_ pixels: Int) -> Data {
    ...
    // Warm, like the menu bar icon while boosted.
    NSGradient(colors: [NSColor(srgbRed: 1, green: 0.72, blue: 0.2, alpha: 1), NSColor(srgbRed: 0.96, green: 0.42, blue: 0.1, alpha: 1)])!
        .draw(in: shape, angle: -90)

    let config = NSImage.SymbolConfiguration(pointSize: 430 * scale, weight: .semibold)
        .applying(.init(paletteColors: [.white]))
    if let symbol = NSImage(systemSymbolName: "sun.max.fill", accessibilityDescription: nil)?.withSymbolConfiguration(config) {
