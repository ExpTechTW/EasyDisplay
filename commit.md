...
```
feat(boost): drive the built-in backlight up to 1000 nits
└┬─┘ └──┬─┘  └──────────────────┬──────────────────┘
 type  scope                   摘要
```
...
選填，小寫，對應功能區：`brightness` `boost` `ddc` `monitor` `menu` `settings` `l10n`。
跨多個區的改動就**不要寫 scope**。
...
```
New(zh-Hant): 內建螢幕可以增亮到 1000 nit
└┬┘ └──┬──┘  └──────────┬─────────┘
分類    語系            條目文字
```
...
```
✗ New(zh-Hant): 寫入 IOMFBBrightnessLevel 和 BLNitsCap 繞過 corebrightnessd
✓ New(zh-Hant): 內建螢幕可以增亮到 1000 nit，色彩不變
```
...
New(zh-Hant): 內建螢幕可以增亮到 1000 nit，色彩不變
New(en-US): the built-in display can be boosted to 1000 nits without changing its colors
New(ja-JP): 内蔵ディスプレイを色を変えずに 1000 nit まで明るくできます
```

```
fix(ddc): find external monitors behind the display coprocessor

Fix(zh-Hant): 修正外接螢幕顯示「不支援亮度控制」
Fix(en-US): external monitors no longer show as unable to change brightness
```
