...
- **バックライト**：`BacklightDriver` がメインスレッドではなく専用のキューで保ちます。明るさのプロパティが変わるたびに framebuffer から通知が届くので、ほかから書き換えられたら（スリープ解除、プリセットの切り替え、True Tone）約 1 ms 以内、画面に出る前に書き戻します。ディスプレイがオフのときは何もせず、スリープ解除ではそれまでの明るさへそのまま戻ります。アイドルでシステムが暗くするときは、なめらかに一緒に暗くなります。ブーストをオンにするときは画面の明るさを保ち、EDR ヘッドルームが変わっても白が同じになるようバックライトを合わせます。オフにするときは corebrightnessd が落ち着くのを待ってから、その明るさへ 1 回でなめらかに移ります。新しい明るさへは、1 秒に 30 回、対数目盛りで 1 段ずつ近づきます。それ以外の処理は 1 秒ごとのセンサーのサンプルとともに行い、スライダと明るさキーはすぐに反映します。
