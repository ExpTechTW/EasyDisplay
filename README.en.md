...
- **Backlight**: `BacklightDriver` holds it on a queue of its own, off the main thread. 30 times a second it reads the backlight once. If something else wrote it (wake, a preset change, True Tone), it writes it back within a tick, about 10–20 ms. If the display is off, it waits. On the way to a new brightness it moves one eased step on a log scale. Everything else runs once a second with the sensor sample, or at once for the slider and keys.
