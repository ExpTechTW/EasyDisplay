...
@Suite struct UpdateCheckTests {
    private func release(
        ...
        let assets = archive ? [Release.Asset(
            name: "EasyDisplay-\(label).zip", size: 1, digest: nil,
            downloadURL: URL(string: "https://example.com/EasyDisplay-\(label).zip")!
        )] : []
        return Release(
    ...
    @Test func decodesTheGitHubReleaseList() throws {
        ...
        [{
          ...
          "html_url": "https://github.com/ExpTechTW/EasyDisplay/releases/tag/26w39a",
          "body": "_快照_\r\n\r\n<!--  easydisplay-build:126000042  -->\r\n",
          "assets": [
            {"name": "EasyDisplay-26w39a.zip", "size": 1234,
             "digest": "sha256:2151b604e3429bff440b9fbc03eb3617bc2603cda96c95b9bb05277f9ddba255",
             "browser_download_url": "https://github.com/ExpTechTW/EasyDisplay/releases/download/26w39a/EasyDisplay-26w39a.zip"},
          ...
          "html_url": "https://github.com/ExpTechTW/EasyDisplay/releases/tag/v26.1", "assets": []
        }]
        """#
    ...
    @Test func readsTheBuildFromInfoPlist() {
        let build = BuildInfo(info: [
            "CFBundleVersion": "126000042", "EasyDisplayLabel": "26.1", "EasyDisplayDate": "26-09-24",
            "EasyDisplayPrerelease": false, "EasyDisplayRepository": "ExpTechTW/EasyDisplay",
        ])
        #expect(build == BuildInfo(info: build.infoForTest))
        #expect(build.code == 126000042)
        #expect(build.label == "26.1")
        #expect(!build.isPrerelease)
        #expect(build.repository == "ExpTechTW/EasyDisplay")
    }
    ...
    @Test func aBuildWithoutAVersionDoesNotUpdate() {
        ...
        let odd = BuildInfo(info: ["EasyDisplayLabel": "26w39a", "CFBundleVersion": "126000001", "EasyDisplayRepository": "a/b/../c"])
...
private extension BuildInfo {
    var infoForTest: [String: Any] {
        var info: [String: Any] = ["CFBundleVersion": String(code), "EasyDisplayLabel": label, "EasyDisplayPrerelease": isPrerelease]
        info["EasyDisplayDate"] = date
        info["EasyDisplayRepository"] = repository
        return info
    }
}

@Suite struct UpdateInstallerTests {
    private let identifier = "io.github.yuyu1015.EasyDisplay"

    private func temporaryFolder() throws -> URL {
        let folder = FileManager.default.temporaryDirectory.appendingPathComponent("EasyDisplayTests-\(UUID().uuidString)")
    ...
    private func makeApp(in folder: URL, version: String = "126000042", identity: String? = nil) throws -> URL {
        let app = folder.appendingPathComponent("EasyDisplay.app")
        let macOS = app.appendingPathComponent("Contents/MacOS")
        try FileManager.default.createDirectory(at: macOS, withIntermediateDirectories: true)
        try FileManager.default.copyItem(at: URL(fileURLWithPath: "/usr/bin/true"), to: macOS.appendingPathComponent("EasyDisplay"))
        let info: NSDictionary = [
            "CFBundleIdentifier": identifier, "CFBundleExecutable": "EasyDisplay", "CFBundleVersion": version,
    ...
    private func asset(for file: URL, digest: String?) throws -> Release.Asset {
        ...
        return Release.Asset(name: "EasyDisplay-26w39a.zip", size: size, digest: digest, downloadURL: file)
    }

    @Test func acceptsOnlyTheDownloadGitHubDescribed() throws {
        let folder = try temporaryFolder()
        defer { try? FileManager.default.removeItem(at: folder) }
        let file = folder.appendingPathComponent("EasyDisplay.zip")
        try Data("EasyDisplay".utf8).write(to: file)
        let hex = SHA256.hash(data: Data("EasyDisplay".utf8)).map { String(format: "%02x", $0) }.joined()
        ...
        #expect(throws: UpdateFailure.damaged) {
            let wrongSize = Release.Asset(name: "EasyDisplay.zip", size: 1, digest: nil, downloadURL: file)
    ...
    @Test func unpacksTheApp() throws {
        ...
        let archive = folder.appendingPathComponent("EasyDisplay.zip")
        ...
        #expect(unpacked.lastPathComponent == "EasyDisplay.app")
