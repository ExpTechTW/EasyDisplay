...
@Suite struct MonitorDatabaseTests {
    private func database(_ folder: URL? = nil) throws -> (MonitorDatabase, URL) {
        let folder = folder ?? FileManager.default.temporaryDirectory.appendingPathComponent("EasyDisplayTests-\(UUID().uuidString)")
    ...
    @Test func theFileIsSetUpForItsJob() throws {
        ...
        let source = try String(contentsOf: URL(fileURLWithPath: #filePath)
            ...
            .appendingPathComponent("Sources/EasyDisplay/System/MonitorDatabase.swift"), encoding: .utf8)
