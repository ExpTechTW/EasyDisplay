...
@Suite struct UpdateCheckTests {
    private func release(
        ...
        let assets = archive ? [Release.Asset(
            name: "EazyDisplay-\(label).zip", size: 1, digest: nil,
            downloadURL: URL(string: "https://example.com/EazyDisplay-\(label).zip")!
        )] : []
        return Release(
    ...
    @Test func decodesTheGitHubReleaseList() throws {
        ...
        [{
          ...
          "html_url": "https://github.com/ExpTechTW/EazyDisplay/releases/tag/26w39a",
          "body": "_快照_\r\n\r\n<!--  eazydisplay-build:126000042  -->\r\n",
          "assets": [
            {"name": "EazyDisplay-26w39a.zip", "size": 1234,
             "digest": "sha256:2151b604e3429bff440b9fbc03eb3617bc2603cda96c95b9bb05277f9ddba255",
             "browser_download_url": "https://github.com/ExpTechTW/EazyDisplay/releases/download/26w39a/EazyDisplay-26w39a.zip"},
          ...
          "html_url": "https://github.com/ExpTechTW/EazyDisplay/releases/tag/v26.1", "assets": []
        }]
        """#
    ...
    @Test func readsTheBuildFromInfoPlist() {
        let build = BuildInfo(info: [
            "CFBundleVersion": "126000042", "EazyDisplayLabel": "26.1", "EazyDisplayDate": "26-09-24",
            "EazyDisplayPrerelease": false, "EazyDisplayRepository": "ExpTechTW/EazyDisplay",
        ])
        #expect(build == BuildInfo(info: build.infoForTest))
        #expect(build.code == 126000042)
        #expect(build.label == "26.1")
        #expect(!build.isPrerelease)
        #expect(build.repository == "ExpTechTW/EazyDisplay")
    }
    ...
    @Test func aBuildWithoutAVersionDoesNotUpdate() {
        ...
        let odd = BuildInfo(info: ["EazyDisplayLabel": "26w39a", "CFBundleVersion": "126000001", "EazyDisplayRepository": "a/b/../c"])
...
private extension BuildInfo {
    var infoForTest: [String: Any] {
        var info: [String: Any] = ["CFBundleVersion": String(code), "EazyDisplayLabel": label, "EazyDisplayPrerelease": isPrerelease]
        info["EazyDisplayDate"] = date
        info["EazyDisplayRepository"] = repository
        return info
    }
}

@Suite struct UpdateInstallerTests {
    private let identifier = "io.github.yuyu1015.EazyDisplay"
    ...
    private func makeApp(in folder: URL, version: String = "126000042", identity: String? = nil) throws -> URL {
        let app = folder.appendingPathComponent("EazyDisplay.app")
        let macOS = app.appendingPathComponent("Contents/MacOS")
        try FileManager.default.createDirectory(at: macOS, withIntermediateDirectories: true)
        try FileManager.default.copyItem(at: URL(fileURLWithPath: "/usr/bin/true"), to: macOS.appendingPathComponent("EazyDisplay"))
        let info: NSDictionary = [
            "CFBundleIdentifier": identifier, "CFBundleExecutable": "EazyDisplay", "CFBundleVersion": version,
    ...
    private func asset(for file: URL, digest: String?) throws -> Release.Asset {
        ...
        return Release.Asset(name: "EazyDisplay-26w39a.zip", size: size, digest: digest, downloadURL: file)
    }

    @Test func acceptsOnlyTheDownloadGitHubDescribed() throws {
        let folder = try temporaryFolder()
        defer { try? FileManager.default.removeItem(at: folder) }
        let file = folder.appendingPathComponent("EazyDisplay.zip")
        try Data("EazyDisplay".utf8).write(to: file)
        let hex = SHA256.hash(data: Data("EazyDisplay".utf8)).map { String(format: "%02x", $0) }.joined()
        ...
        #expect(throws: UpdateFailure.damaged) {
            let wrongSize = Release.Asset(name: "EazyDisplay.zip", size: 1, digest: nil, downloadURL: file)
    ...
    @Test func unpacksTheApp() throws {
        ...
        let archive = folder.appendingPathComponent("EazyDisplay.zip")
        ...
        #expect(unpacked.lastPathComponent == "EazyDisplay.app")
